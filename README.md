...
## About
A LevelUP compatible leaderless multi-master database with eventual consistency, using hyperbee + CRDT + HLC.  Similarly CockroachDB achieves replication on top of RocksDB, but here it is a pure P2P **streaming** database, with zero central management. LevelDB compatibility allows to use Dynalite on top to achieve DynamoDB compatibility with sophisticated auto-updated secondary indexes, and fairly complex queries. Work on @mhart's [Dynalite](https://github.com/tradle/dynalite) is almost completed to remove HTTP server, to make this combination perfect for serverless.
